...
class ColumnSet
{
    ...
    public static function build($input): static
    {
    ...
    public static function fromModelInstance($model): static
    {
        return new static(
    ...
    public static function fromArray($columns): static
    {
    ...
    public function hide($hidden): static
    {
        if (! $hidden) {
            return $this;
        }
        ...
        $this->columns->each(function ($column) use ($hidden) {
            $column->hidden = in_array(Str::after($column->name, '.'), $hidden, true);
        });

        return $this;
    }

    public function formatDates($dates): static
    {
    ...
    public function formatTimes($times): static
    {
    ...
    public function search($searchable): static
    {
        if (! $searchable) {
            return $this;
        }
        ...
        $this->columns->each(function ($column) use ($searchable) {
            $column->searchable = in_array($column->name, $searchable, true);
        });
